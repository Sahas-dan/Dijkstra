...
public class DijkstrasGraph<T> implements GraphADT<T> {
  ...
  protected class Path implements Comparable<Path> {
    ...
    public Path(Path copyPath, Edge extendBy) {
      ...
      this.start = copyPath.start; // sets the new start as the start of the path to be copied
      this.end = extendBy.target; // sets the new end as the end of the path to be copied
      this.dataSequence = new LinkedList<>(); // Initializes a new LinkedList to store the
                                              // new Path's data Sequence
      for (int i = 0; i < copyPath.dataSequence.size(); i++) {
        this.dataSequence.add(new Vertex(copyPath.dataSequence.get(i)).data); // adds all the data
                                                                              // sequence info from
                                                                              // copyPath into the
                                                                              // new Path's data
                                                                              // sequence
      ...
      Vertex vert = extendBy.target; // creates a new vertex with the target of the extendBy edge
      this.dataSequence.add(vert.data); // adds this vertex to the data sequence for the new Path

      this.distance = copyPath.distance + extendBy.weight; // computes the new distance for the new
                                                           // Path
  ...
  protected Path dijkstrasShortestPath(T start, T end) {
    ...
    Vertex first = vertices.get(start); // gets the actual vertex from the hash table
    Vertex last = vertices.get(end); // gets the actual vertex from the hash table
    if (first == null) { // throws exception if cannot be found
      throw new NoSuchElementException();
    }
    if (last == null) { // throws exception if cannot be found
      throw new NoSuchElementException();
    }

    Path beginning = new Path(first); // creates a new path
    ArrayList<Vertex> nodesVisited = new ArrayList<>(); // creates a new arraylist to hold the
                                                        // nodes that are visited
    PriorityQueue<Path> queue = new PriorityQueue<Path>(); // creates a new priority queue
    queue.add(beginning); // adds it to the beginning of the queue

    while (!queue.isEmpty()) {
      Path result = queue.remove(); // removes it from the queue and makes it the result path

      if (result.end.equals(last)) { // returns the result if it is the same as the last node
        return result;
      } else {
        nodesVisited.add(result.end);// adds the end of the last path to the visited list

        for (Edge count : result.end.edgesLeaving) { // for each edge extending from the end
          if (!nodesVisited.contains(count.target)) { // if visited list doesn't have edges target
            queue.add(new Path(result, count)); // add it to the queue
          }
        }
      }

    }
    throw new NoSuchElementException(); // throw a noSuchElementException if there is no result
  }
